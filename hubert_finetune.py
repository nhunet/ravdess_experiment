...
def _repair_pos_conv(hubert, model_name: str = MODEL_NAME) -> bool:
    """
    FIX #8: torch>=2.1 đổi weight_norm → parametrizations (original0/original1),
    nhưng transformers không remap key cũ (weight_g/weight_v) → pos_conv_embed
    bị KHỞI TẠO NGẪU NHIÊN thay vì nạp từ checkpoint.

    Model vẫn chạy và vẫn ra ~80% nên cực dễ bỏ sót — nhưng đó là một HuBERT
    hỏng một phần. Đây chính là nguyên nhân lần chạy v1 bị underfit
    (loss chững ở 0.78). Sau khi vá: loss xuống 0.74 ngay ở epoch 5.

    Hàm này PHẢI được gọi cho MỌI nơi load HuBERT — kể cả frozen baseline —
    nếu không, baseline sẽ bị làm yếu đi một cách giả tạo và mọi so sánh
    đều vô nghĩa.
    """
...
class HubertSER(nn.Module):
    ...
    def __init__(self, n_classes: int,
        ...
        _repair_pos_conv(self.hubert, model_name)      # FIX #8 – bắt buộc
